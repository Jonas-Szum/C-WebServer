...
#include <unistd.h>
extern char** environ;
#define BACKLOG (10) //define how many threads can be running at once

void serve_request(int);
/* because of threading, a global variable is a bad idea
  ...
  if(S_ISREG(isDir.st_mode))
    ...
    if(read_fd != -1) //if index is there, behave normally
      ...
      while((fileStruct = readdir(myDir)) != NULL) //created a listing for every file in the directory
        ...
        strcat(indexListing, newFileName); //insert the link
        struct stat isDir2;
